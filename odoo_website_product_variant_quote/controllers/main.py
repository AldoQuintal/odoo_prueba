...
class OdooWebsiteProductQuote(http.Controller):
	...
	def get_quote_nonlogin(self, **post):
		...
		if is_send_quote == True:
			...
			template_id = request.env.ref('odoo_website_product_variant_quote.email_template_def_request_quotation')
	...
	def quote_confirm(self, **post):
		...
		if is_send_quote == True:
			...
			template_id = request.env.ref('odoo_website_product_variant_quote.email_template_def_request_quotation', raise_if_not_found=False)
			print(f'Tempate id {template_id}')
			print(sale_order_create.partner_id)
			print(sale_order_create.partner_id.email)
			if sale_order_create.partner_id and sale_order_create.partner_id.email:
				template_id.sudo().with_context(email_to=sale_order_create.partner_id.email).send_mail(sale_order_create.id, force_send=True)

		order.sudo().unlink()
		request.session['quote_order_id'] = False
		return request.render("odoo_website_product_variant_quote.quotation_send_successfully")

	def send_quotation_automatic(self, sale_order_create):
		template_id = request.env.ref('odoo_website_product_variant_quote.email_template_request_quotation', raise_if_not_found=False)
